#!/usr/bin/env python3
"""
Update barabara-members.json with new YouTube channel data
"""
import json
import os
import glob
...
CHANNEL_MAPPING = {
    '아오세이준 (Aoseijun)': ['@aoseijun'],
    '아카즈네 카츠키 (Akazune Katsuki)': ['@AkazuneKatsuki'],
    '시로네 하루 (Shirone Haru)': ['@Shironeharu'],
    '네루 쿠로밍 (Neru Kuroming)': ['@NeruKuroming'],
    '시니초코레이토 (Sinichocoreito)': ['@sinichocoreito'],
    '미즈히로 슈우 (Mizuiro Syu)': ['@mizuiroSyu']
}
...
def update_barabara_members():
    """Update barabara-members.json with new YouTube data"""
    members_file = os.path.join(LOCAL_DATA_DIR, 'vuddy', 'comprehensive_analysis', 'barabara-members.json')
    ...
    if os.path.exists(members_file):
        ...
        data = {
            'group_name': 'BARABARA',
            'description': 'BARABARA 소속 VTuber 크리에이터',
            'official_channel': '@BARABARA_KR',
